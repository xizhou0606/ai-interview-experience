...
import { publishedNews } from '../../../data/technology-news'
...
export function SearchDialog({ open, onClose }: SearchDialogProps) {
  ...
  const items = useMemo<SearchItem[]>(() => [
    ...
    ...publishedNews.map((item) => ({ title: item.title, subtitle: `${item.date} · ${item.summary}`, type: `技术新闻 · ${item.category}`, hash: '#news' })),
  ], [apiItems])
  ...
  return (
    <div className="dialog-backdrop" onMouseDown={onClose} role="presentation">
      <section className="search-dialog" role="dialog" aria-modal="true" aria-label="全站搜索" onMouseDown={(event) => event.stopPropagation()}>
        <div className="search-input-row"><Search size={19} /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="输入 AI SDK、RAG、技术新闻…" /><button onClick={onClose}>ESC</button></div>
        <div className="search-meta"><span>{query ? `找到 ${filtered.length} 个结果` : '推荐内容'}</span><span>技术栈 · 工程实践 · 官方 API · 技术新闻</span></div>
