...
export function SearchDialog({ open, onClose }: SearchDialogProps) {
  ...
  const items = useMemo<SearchItem[]>(() => [
    { title: '技术新闻', subtitle: '近两周可核验的前端、后端与 AI 发布', type: '栏目', hash: '#news' },
    ...
    ...publishedNews().map((item) => ({ title: item.title, subtitle: item.summary, type: `技术新闻 · ${item.category}`, hash: '#news' })),
    ...apiItems,
  ...
  return (
    <div className="dialog-backdrop" onMouseDown={onClose} role="presentation">
      <section className="search-dialog" role="dialog" aria-modal="true" aria-label="全站搜索" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="search-meta"><span>{query ? `找到 ${filtered.length} 个结果` : '推荐内容'}</span><span>技术栈 · 技术新闻 · 官方 API · 项目证据</span></div>
        <div className="search-results">
          {filtered.map((item) => <button key={`${item.type}-${item.title}`} onClick={() => openItem(item.hash)}><span className="search-result-icon">{item.type === '真实项目' ? <FolderGit2 size={17} /> : item.type === '工程模式' ? <Workflow size={17} /> : item.type.includes('技术新闻') || item.type === '栏目' ? <Newspaper size={17} /> : item.type.includes('API') ? <Braces size={17} /> : <Boxes size={17} />}</span><span><strong>{item.title}</strong><small>{item.subtitle}</small></span><span className="search-type">{item.type}</span></button>)}
