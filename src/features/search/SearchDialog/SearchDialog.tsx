...
import { TECHNOLOGY_NEWS, isFreshNews } from '../../../data/technology-news'
...
export function SearchDialog({ open, onClose }: SearchDialogProps) {
  ...
  const items = useMemo<SearchItem[]>(() => [
    ...TECHNOLOGY_NEWS.filter((item) => isFreshNews(item)).map((item) => ({ title: item.title, subtitle: item.summary, type: `技术新闻 · ${item.category}`, hash: '#news' })),
    ...
    <div className="dialog-backdrop" onMouseDown={onClose} role="presentation">
      <section className="search-dialog" role="dialog" aria-modal="true" aria-label="全站搜索" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="search-results">
          {filtered.map((item) => <button key={`${item.type}-${item.title}-${item.hash}`} onClick={() => openItem(item.hash)}><span className="search-result-icon">{item.type.startsWith('技术新闻') ? <Newspaper size={17} /> : item.type === '真实项目' ? <FolderGit2 size={17} /> : item.type === '工程模式' ? <Workflow size={17} /> : item.type.includes('API') ? <Braces size={17} /> : <Boxes size={17} />}</span><span><strong>{item.title}</strong><small>{item.subtitle}</small></span><span className="search-type">{item.type}</span></button>)}
