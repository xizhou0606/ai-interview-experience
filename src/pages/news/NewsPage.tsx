...
export function NewsPage() {
  ...
  return (
    <DocsShell active="news" toc={groups.length ? groups.map((group) => group.date) : undefined}>
      <PageIntro
        kicker="TECH NEWS"
        title="近两周前端、后端与 AI 技术新闻"
        description="只收录可核验的一手来源。按日分组，标题和摘要尽量一眼看完；条目按链接去重，并只保留约两周窗口。"
      ...
      {groups.length === 0 && (
        <div className="news-empty"><Newspaper size={20} /><div><strong>这个分类暂时没有条目</strong><p>近两周窗口里还没有匹配的新闻。换一个分类，或等下一轮扫描写入。</p></div></div>
      )}
    </DocsShell>
  )
}
